  ...
  return <div className="u-page"><div className="u-tabs">{tabs.map(([k, l]) => <button key={k} className={tab === k ? 'active' : ''} onClick={() => setTab(k)}>{l}</button>)}</div><div className={['backup', 'maintenance'].includes(tab) ? 'u-settings-shell' : 'card u-panel'}>
    ...
    {tab === 'backup' && <div className="u-settings-stack">
      ...
      <section className="card u-panel u-settings-card">
        ...
        {s.updates?.update_mode !== 'notify' ? <p className="u-note">{t('Main versions change the first or second version number, such as 1.4.x → 1.5.0. Patch releases such as 1.4.1 → 1.4.2 are skipped.')}</p> : <p className="u-note">{t('Matching releases only send one notice. Installation starts only after you review and confirm it manually.')}</p>}
