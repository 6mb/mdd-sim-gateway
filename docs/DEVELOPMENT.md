...
归档或部署。曾经出现过两个 clone 各自推进同名分支，导致一边提到的 commit 在另一边
「不存在」——排查它花的时间远超过统一副本的成本。
...
**每一次提交前都跑，不是每一批改动跑一次。** 真实 IMSI 曾经在「上次扫描之后、这次提交
之前」写入测试文件，因而躲过检查进了 commit，只能重写历史清除。
...
**先验证再断言。** 不确定的事就去查，查不到就说查不到，不要用推测填空，也不要因为一个
异常读数就断言故障——曾经把用户自己在界面上配好并成功运行的功能误报成异常。
