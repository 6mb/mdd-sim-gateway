...
- 升级停在 `engine_image`：确认 Docker daemon 可访问 `ghcr.io` 与
  `pkg-containers.githubusercontent.com`，并检查 `update/engine-pull.log`。不要在 Pi 上
  改为源码编译绕过；修复 GHCR 网络后重试，旧 Engine 在新镜像通过完整身份检查前不会被替换。
