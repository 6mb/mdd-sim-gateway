...
4G 模块收到的短信写入数据库后，默认从 Modem/SIM 存储中删除，以免存储写满后无法再收短信。可在控制服务的环境中设置 `MDD_CELLULAR_SMS_STORAGE`：`delete`（默认，入库后删除）、`when_full`（保留在模块中，仅在存储将满时删除最早的已入库短信）或 `keep`（始终保留）。`when_full` 通过 `AT+CPMS?` 读取容量，需要 ModemManager 以 `--debug` 运行；否则按 `MDD_CELLULAR_SMS_STORAGE_LIMIT`（默认 20 条）计算。配置文件 `settings.cellular_sms_storage` 若已设置，则优先于环境变量。
