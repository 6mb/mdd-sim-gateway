...
控制面不依赖浏览器登录，每 6 小时检查一次 Release，并通过已启用的 Webhook、Telegram 或 PushPlus 通道发送一次去重通知。通知策略可选择所有新版本，或仅在主版本/次版本变化时通知；例如后者不会通知 `1.4.1 → 1.4.2`。
